---
id: q-bigdata-clickhouse-0012
title: ClickHouse 的性能模型面试题应该怎么讲到原理层？
domain: bigdata
...
claim_ids:
  ...
  - bigdata-clickhouse-claim-0013
related_docs:
  - bigdata/clickhouse/performance-model
estimated_minutes: 10
---

# 题目

ClickHouse 的性能模型面试题应该怎么讲到原理层？

# 一句话结论

性能瓶颈常见于排序键设计差、分区过细、小 part 过多、join 内存高、分布式查询网络开销和低选择性扫描。 这类题要从对象、状态、链路和边界回答，才能体现原理深度。

# 核心机制

1. 性能瓶颈常见于排序键设计差、分区过细、小 part 过多、join 内存高、分布式查询网络开销和低选择性扫描。
2. Partition、ORDER BY、primary key、granule、part 和 shard/replica 布局决定查询裁剪、merge 成本和分布式查询表现。
3. 调优要围绕批量写入、ORDER BY、PARTITION BY、数据跳过索引、物化视图、TTL 和集群拓扑做取舍。
4. 本主题重点是：说明吞吐、延迟、并发、内存、网络、磁盘或优化器的主要瓶颈。
