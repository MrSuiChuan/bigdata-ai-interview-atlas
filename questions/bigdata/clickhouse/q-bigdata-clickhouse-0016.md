---
id: q-bigdata-clickhouse-0016
title: ClickHouse 的可观测性面试题应该怎么讲到原理层？
domain: bigdata
...
claim_ids:
  ...
  - bigdata-clickhouse-claim-0017
related_docs:
  - bigdata/clickhouse/observability
estimated_minutes: 10
---

# 题目

ClickHouse 的可观测性面试题应该怎么讲到原理层？

# 一句话结论

system 表、query_log、part_log、merge 相关指标、replication queue、慢查询和磁盘水位要联合分析。 这类题要从对象、状态、链路和边界回答，才能体现原理深度。

# 核心机制

1. system 表、query_log、part_log、merge 相关指标、replication queue、慢查询和磁盘水位要联合分析。
2. 性能瓶颈常见于排序键设计差、分区过细、小 part 过多、join 内存高、分布式查询网络开销和低选择性扫描。
3. 排障先看查询计划、扫描行数、parts 数量、merge backlog、内存、分布式网络和副本队列。
4. 本主题重点是：说明指标、日志、计划、UI、系统表或命令如何联合定位。
