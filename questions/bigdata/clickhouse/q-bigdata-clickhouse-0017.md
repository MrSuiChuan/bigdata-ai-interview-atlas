---
id: q-bigdata-clickhouse-0017
title: ClickHouse 的排障面试题应该怎么讲到原理层？
domain: bigdata
...
claim_ids:
  ...
  - bigdata-clickhouse-claim-0018
related_docs:
  - bigdata/clickhouse/troubleshooting
estimated_minutes: 10
---

# 题目

ClickHouse 的排障面试题应该怎么讲到原理层？

# 一句话结论

排障先看查询计划、扫描行数、parts 数量、merge backlog、内存、分布式网络和副本队列。 这类题要从对象、状态、链路和边界回答，才能体现原理深度。

# 核心机制

1. 排障先看查询计划、扫描行数、parts 数量、merge backlog、内存、分布式网络和副本队列。
2. system 表、query_log、part_log、merge 相关指标、replication queue、慢查询和磁盘水位要联合分析。
3. 副本、分片、Distributed 表和后台 merge 的问题要分开看；单副本表和复制表的恢复边界不同。
4. 本主题重点是：说明典型故障如何按层拆解，而不是直接重启或重跑。
